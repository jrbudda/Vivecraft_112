...
public class OpenVRPlayer 
{
	...
	public void setFreeMove(boolean free) { 
		...
		if(free != was){
			CPacketCustomPayload pack =	NetworkHelper.getVivecraftClientPacket(NetworkHelper.PacketDiscriminators.MOVEMODE, isFreeMoveCurrent ?  new byte[]{1} : new byte[]{0});
	...
	public Vec3d AimedPointAtDistance(VRData source, int controller, double distance) {
        ...
        return vec3d.addVector(vec3d1.x * distance, vec3d1.y * distance, vec3d1.z * distance);
	}
