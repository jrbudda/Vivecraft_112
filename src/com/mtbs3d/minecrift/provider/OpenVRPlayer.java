...
public class OpenVRPlayer 
{
    ...
    public void snapRoomOriginToPlayerEntity(EntityPlayerSP player, boolean reset)
    {
    	...
    	if(player.isRiding()){
    		x = player.getRidingEntity().posX - campos.x;
    		y = player.getRidingEntity().posY + player.getRidingEntity().getMountedYOffset();
    		z = player.getRidingEntity().posZ - campos.z;
    	}
   	...
   	public void checkandUpdateRotateScale(){
		...
		Vec3d o = vrdata_world_pre.hmd.getPosition();

		if(!mc.isGamePaused()){
			mc.vrSettings.vehicleRotation = true;
			if(mc.vrSettings.vehicleRotation && mc.player.isRiding()){
				Entity e = mc.player.getRidingEntity();		
				float end = e.rotationYaw;
				if (e instanceof EntityMinecart) end = getMinecartRenderYaw((EntityMinecart) e);
				float start = vrot;			
		        float difference = Math.abs(end - start);
				...
				if(difference < 120 ){
					mc.vrSettings.vrWorldRotation -= end - start;
					mc.vrSettings.vrWorldRotation = mc.vrSettings.vrWorldRotation % 360;
					MCOpenVR.seatedRot = mc.vrSettings.vrWorldRotation;
				}
		        
				o = e.getPositionVector();
				vrot = end;  
			}
		}
				
		if (((float)Math.toRadians(mc.vrSettings.vrWorldRotation)) != vrdata_world_pre.rotation || worldScale != vrdata_world_pre.scaleY) {
			Vec3d pt = vrdata_world_pre.origin;
			
			float end = (float) Math.toRadians(mc.vrSettings.vrWorldRotation);
			float start = vrdata_world_pre.rotation;
				
			float rads = -end+start; //reverse rotate.
	...
	private float getMinecartRenderYaw(EntityMinecart entity){
		...
		Vec3d vec3d =  new Vec3d(entity.posX - entity.lastTickPosX, 0, entity.posZ-entity.lastTickPosZ);
		if(vec3d.lengthVector() < 0.001){
			return vrot;
		}
		float out = (float)(Math.atan2(-vec3d.x, vec3d.z) * 180.0D / Math.PI);
		return Math.round(out / 10) * 10;
		
//		if (vec3d != null)
//		{
//			Vec3d vec3d1 = entity.getPosOffset(vec3d.x, vec3d.y, vec3d.z, 0.30000001192092896D);
//			Vec3d vec3d2 = entity.getPosOffset(vec3d.x, vec3d.y, vec3d.z, -0.30000001192092896D);
//
//			if (vec3d1 == null)
//			{
//				vec3d1 = vec3d;
...
//			}
//
//			//            x += vec3d.x - entity.posX;
//			//            y += (vec3d1.y + vec3d2.y) / 2.0D - entity.posY;
//			//            z += vec3d.z - entity.posZ;
//			Vec3d vec3d3 = vec3d2.addVector(-vec3d1.x, -vec3d1.y, -vec3d1.z);
//
//			if (vec3d3.lengthVector() != 0.0D)
//			{
//				vec3d3 = vec3d3.normalize();
//				float out = (float)(Math.atan2(vec3d3.z, vec3d3.x) * 180.0D / Math.PI) ;
//				if (out < 0) {
//					 out = (float)(Math.atan2(-vec3d3.z, -vec3d3.x) * 180.0D / Math.PI);
//				}
//				out = (out + 360) % 360;
//				System.out.println(vec3d3 + " " + out);
//				return out;
//			}
//
//		}
//		System.out.println("raw : " + entity.rotationYaw);
//		return vrot;
	}
	...
	public void onGuiScreenChanged(GuiScreen previousScreen, GuiScreen newScreen, boolean unpressKeys)
		{
			...
			// main menu/win game/
			if (MCOpenVR.mc.world==null || newScreen instanceof GuiWinGame ) {
