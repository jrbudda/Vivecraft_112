...
public class VRInputAction {
	...
	public void pressKey() {
		int awtCode = KeyboardSimulator.translateToAWT(keyBinding.getKeyCode());

		if (Display.isActive() && awtCode != Keyboard.KEY_NONE && !MCOpenVR.isSafeBinding(keyBinding) && (!Reflector.forgeExists() || Reflector.call(keyBinding, Reflector.ForgeKeyBinding_getKeyModifier) == Reflector.getFieldValue(Reflector.KeyModifier_NONE))) {
			try { // because apparently java is just stupid
				//System.out.println("Keyboard Simulator keyPress: " + ", LWJGL code: " + keyBinding.getKeyCode() + ", AWT code: " + awtCode);
				KeyboardSimulator.robot.keyPress(awtCode);
				return;
			} catch (Exception e) {
				System.out.println("Key error: " + e.toString() + ", LWJGL code: " + keyBinding.getKeyCode() + ", AWT code: " + awtCode);
			}
		}

		// If all else fails, just press the binding directly
		//System.out.println("setKeyBindState true: " + ", LWJGL code: " + keyBinding.getKeyCode() + ", AWT code: " + awtCode);
	...
	public void unpressKey() {
		...
		if (Display.isActive() && awtCode != Keyboard.KEY_NONE && !MCOpenVR.isSafeBinding(keyBinding) && (!Reflector.forgeExists() || Reflector.call(keyBinding, Reflector.ForgeKeyBinding_getKeyModifier) == Reflector.getFieldValue(Reflector.KeyModifier_NONE))) {
			try { // because apparently java is just stupid
				//System.out.println("Keyboard Simulator keyRelease: " + ", LWJGL code: " + keyBinding.getKeyCode() + ", AWT code: " + awtCode);
		...
		// If all else fails, just press the binding directly
		//System.out.println("unpressKey: " + ", LWJGL code: " + keyBinding.getKeyCode() + ", AWT code: " + awtCode);
